...
export class GoogleAIClient extends LLMClient {
  ...
  const apiKey = process.env.GOOGLE_API_KEY;
    ...
    this.model = this.genai.getGenerativeModel({ model }, { timeout: 120000 });
...
export class MockLLMClient extends LLMClient {
  ...
  async chat(messages: Message[]): Promise<string> {
    console.log("MockLLMClient received messages:", messages.length);
    return JSON.stringify({
      summary: "This is a mock analysis response for testing purposes.",
      complexity: {
        score: 5,
        reasoning: "Mock complexity reasoning."
      },
      vulnerabilities: [
        {
...
export type ProviderName = 'google' | 'groq' | 'ollama' | 'anthropic' | 'openai' | 'mock';
