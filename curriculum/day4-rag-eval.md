...
1.  Type: Code Units (Functions/Methods/Classes)
    *   Chunking strategy: Syntax-aware logical blocks (via regex/AI).
    *   Chunk size: Variable (1 unit per chunk).
    *   Metadata: functionName, signature, filePath, dependencies (imports used within), exports (is it public?), startLine, endLine.
2.  Type: Interfaces & Type Definitions
    *   Chunking strategy: Full definition.
    *   Metadata: typeName, filePath, relatedClasses.
3.  Type: File Overviews (Synthetic)
    *   Chunking strategy: 1 per file. Generated by AI.
    *   Content: A 2-3 sentence summary of the file's responsibility + list of main exports.
    *   Why: Essential for "Where is X configured?" queries where the file contains many small, non-descriptive variables.
Query Handling
*   Code-specific: Uses BM25 (Keyword Search) on metadata fields like functionName and filePath.
*   Conceptual: Uses Vector Search against the "File Overviews" and docstrings.
*   Multi-file: Agentic Step-Expansion. If the first search finds a function that imports AuthService, the system performs one targeted follow-up search for that specific service definition.
Retrieval Strategy
*   Vector search configuration: Cosine similarity using a code-optimized embedding model (e.g., voyage-code-2).
*   Hybrid search: Required. Weight: 70% Keyword / 30% Vector. Codebases rely heavily on exact matches for symbols (identifiers).
*   Reranking approach: Use a Cross-Encoder reranker (like Cohere Rerank). 
    *   Process: Retrieve 50 chunks -> Rerank -> Send top 10 to LLM. This significantly reduces "noise" in the context window.
Generation
*   System prompt considerations: 
    *   "You are a codebase analyzer. ONLY use the provided context."
    *   "If the implementation of a called function is not in the context, state 'Implementation not found'—do NOT guess the logic."
    *   "Format: Answer in Markdown. Include code blocks with file path headers."
*   Citations: Every claim must be followed by [File: path/to/file.ts, Line: 12].
...
[Keyword Search (BM25)] + [Vector Search]
     ...
     └───────────┬───────────┘
          ...
          [Top 50 Chunks]
                 │
                 ▼
          [Reranker Model] ──▶ (Filtered Precision)
                 │
                 ▼
          [LLM Generation] ──▶ (Strictly Grounded Answer)
Potential Issues
1.  Stale Index: Code changes frequently; the RAG index can become outdated quickly.
2.  Token Fragmentation: A function might be 200 lines, exceeding the chunk size and losing its closing bracket or final logic.
3.  Common Names: init() or handler() appearing in 50 files (Keyword search needs to be file-path aware).
Metrics to Track
1.  Retrieval Precision@3: How often is the "true" answer in the top 3 results?
2.  Faithfulness (RAGAS): Measures how often the answer is derived strictly from the context (catches inference/hallucinations).
3.  Search Latency: Reranking adds time; we must ensure the UX remains snappy.
